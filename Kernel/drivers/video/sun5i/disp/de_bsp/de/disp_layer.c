...
__s32 BSP_disp_layer_wait_reg_load(__u32 sel)
{
    
    __u32 timeout = (100 * HZ)/1000;//100ms

    init_waitqueue_head(&(gdisp.screen[sel].screen_queue));
    gdisp.screen[sel].b_screen_finished = 1;
    timeout = wait_event_interruptible_timeout(gdisp.screen[sel].screen_queue, 
                                  gdisp.screen[sel].b_screen_finished == 2, timeout);
    gdisp.screen[sel].b_screen_finished = 0;
    if(timeout == 0)
    {
        __wrn("wait screen %d timeout\n", sel);
        return DIS_FAIL;
    }
