...
#define EINK_INIT_MODE  0x01
#define EINK_DU_MODE    0x02
#define EINK_GC16_MODE  0x04
#define EINK_GC4_MODE   0x08
#define EINK_A2_MODE    0x10
#define EINK_LOCAL_MODE 0x80
...
static int Fb_ioctl(struct fb_info *info, unsigned int cmd,unsigned long arg)
{
	...
	switch (upd_data.waveform_mode) {
	  case WF_DU: a13wf = EINK_DU_MODE; break;
	  case WF_GC16: a13wf = EINK_GC16_MODE; break;
	  case WF_GC4: a13wf = EINK_GC16_MODE; break;
	  case WF_INIT: a13wf = EINK_INIT_MODE; break;
	  case WF_A2: a13wf = EINK_A2_MODE; break;
	  default: a13wf = 0; break;
	}
	if (! a13wf) break;

	if (upd_data.update_mode == UPDATE_MODE_PARTIAL) {
		a13wf |= EINK_LOCAL_MODE;
	}

	Disp_eink_set_orientation(0, 0, info->var.rotate);
	Disp_eink_update(0, 0, a13wf);
...
{    
    ...
    if(from == 0)//call from lcd driver
    {
        ...
        for(i=0; i<8; i++)
        {
        	...
        	g_fbi.fbinfo[i]->var.yres            = 600;
        	g_fbi.fbinfo[i]->var.xres_virtual    = 800;
        	g_fbi.fbinfo[i]->var.yres_virtual    = 600*2;
            /* 
            	g_fbi.fbinfo[i]->var.xres            = 800;
    ...
    if(g_fbi.disp_init.b_init)
    {
        ...
        for(i = 0; i<fb_num; i++)
        {
            ...
            */
            ...
            fb_para.width = 800;
            fb_para.height = 600;
            fb_para.output_width = 800;
            fb_para.output_height = 600;
