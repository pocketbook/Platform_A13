...
{
	unsigned int timeout = 0x1fe;//0xff;
	aw_twi_set_start(base_addr);
	aw_twi_clear_irq_flag(base_addr);
	while((1 == aw_twi_get_start(base_addr))&&(--timeout));
	if(timeout == 0) {
...
static int i2c_sunxi_suspend(struct platform_device *pdev,  pm_message_t state)
{
	...
	if(0 == i2c->bus_num) {
		i2c->suspend_flag = 0;
		int timeout = 5; //x10 ms
		//wait while transfer is not complete.
		while( i2c->status != I2C_XFER_IDLE && timeout-- ) {
			i2c_dbg("[i2c%d] wait going to suspend while xfer is not complete, dev addr = 0x%x\n", i2c->adap.nr, i2c->msg? i2c->msg->addr : 0xff);
			msleep(10);
		}
		return 0;
	}

#if defined(CONFIG_TOUCHSCREEN_FT5X_TS) || defined (CONFIG_TOUCHSCREEN_FT5X_TS_PB) || defined (CONFIG_TOUCHSCREEN_CNTOUCH_PB) \
	|| defined(CONFIG_TOUCHSCREEN_CYPRESS_XRZ)
	/*
	 * if device is going to PARTIAL sleep mode,
	 * then don't suspend 2-nd i2c bus, for a faster resume by touch.
	 */
	if (2 == i2c->bus_num && g_suspend_state == PM_SUSPEND_PARTIAL) {
		i2c->suspend_flag = 0;
		return 0;
	}
#endif
...
static int i2c_sunxi_resume(struct platform_device *pdev)
{
	...
	|| defined(CONFIG_TOUCHSCREEN_CYPRESS_XRZ)
	/*
	 * Resume only suspended 2-nd i2c bus
	 */
	if (i2c->bus_num == 2 && !i2c->suspend_flag) {
		i2c->suspend_flag = 0;
		return 0;
	}
#endif
