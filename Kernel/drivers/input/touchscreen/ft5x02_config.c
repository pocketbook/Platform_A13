...
extern int ft5x02_i2c_Read(struct i2c_client *client, char *writebuf, int writelen,
...
int ft5x02_Init_IC_Param_qtw(struct i2c_client *client)
{
	int err = 0;
	int i = 0;
	...
	for (i = 0; i < g_ft5x02_tx_num_qtw; i++) {
		if (g_ft5x02_tx_order_qtw[i] != 0xFF) {
			/*set tx order*/
			err = ft5x02_set_tx_order(client, i, g_ft5x02_tx_order_qtw[i]);
			if (err < 0) {
				dev_err(&client->dev, "%s:could not set tx%d order.\n",
						__func__, i);
				goto RETURN_WORK;
		...
		/*set tx cap*/
		err = ft5x02_set_tx_cap(client, i, g_ft5x02_tx_cap_qtw[i]);
		if (err < 0) {
			dev_err(&client->dev, "%s:could not set tx%d cap.\n",
					__func__, i);
			goto RETURN_WORK;
	...
	/*set tx offset*/
	err = ft5x02_set_tx_offset(client, 0, g_ft5x02_tx_offset_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set tx 0 offset.\n",
				__func__);
		goto RETURN_WORK;
	...
	/*set rx offset and cap*/
	for (i = 0; i < g_ft5x02_rx_num_qtw; i++) {
		/*set rx order*/
		err = ft5x02_set_rx_order(client, i, g_ft5x02_rx_order_qtw[i]);
		if (err < 0) {
			dev_err(&client->dev, "%s:could not set rx%d order.\n",
					__func__, i);
			goto RETURN_WORK;
		...
		/*set rx cap*/
		err = ft5x02_set_rx_cap(client, i, g_ft5x02_rx_cap_qtw[i]);
		if (err < 0) {
			dev_err(&client->dev, "%s:could not set rx%d cap.\n",
					__func__, i);
			goto RETURN_WORK;
	...
	for (i = 0; i < g_ft5x02_rx_num_qtw/2; i++) {
		err = ft5x02_set_rx_offset(client, i*2, g_ft5x02_rx_offset_qtw[i]>>4);
		if (err < 0) {
			dev_err(&client->dev, "%s:could not set rx offset.\n",
				__func__);
			goto RETURN_WORK;
		}
		err = ft5x02_set_rx_offset(client, i*2+1, g_ft5x02_rx_offset_qtw[i]&0x0F);
		if (err < 0) {
			dev_err(&client->dev, "%s:could not set rx offset.\n",
				__func__);
			goto RETURN_WORK;
	...
	/*set scan select*/
	err = ft5x02_set_scan_select(client, g_ft5x02_scanselect_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set scan select.\n",
			__func__);
		goto RETURN_WORK;
	...
	/*set tx number*/
	err = ft5x02_set_tx_num(client, g_ft5x02_tx_num_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set tx num.\n",
			__func__);
		goto RETURN_WORK;
	...
	/*set rx number*/
	err = ft5x02_set_rx_num(client, g_ft5x02_rx_num_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set rx num.\n",
			__func__);
		goto RETURN_WORK;
	...
	/*set gain*/
	err = ft5x02_set_gain(client, g_ft5x02_gain_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set gain.\n",
			__func__);
		goto RETURN_WORK;
	...
	/*set voltage*/
	err = ft5x02_set_vol(client, g_ft5x02_voltage_qtw);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set voltage.\n",
			__func__);
		goto RETURN_WORK;
	...
	err = ft5x02_write_reg(client, FT5X02_REG_ADC_TARGET_HIGH,
			g_param_ft5x02_qtw.ft5x02_ADC_TARGET>>8);
	if (err < 0) {
		dev_err(&client->dev, "%s:write ADC_TARGET_HIGH failed.\n", __func__);
		return err;
	}
	err = ft5x02_write_reg(client, FT5X02_REG_ADC_TARGET_LOW,
			g_param_ft5x02_qtw.ft5x02_ADC_TARGET);
	if (err < 0) {
		dev_err(&client->dev, "%s:write ADC_TARGET_LOW failed.\n", __func__);
		return err;
	}
...
RETURN_WORK:	
	...
	/*set resolution*/
	err = ft5x02_set_Resolution(client, g_param_ft5x02_qtw.ft5x02_RESOLUTION_X,
				g_param_ft5x02_qtw.ft5x02_RESOLUTION_Y);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set resolution.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set face detect pre value*/
	err = ft5x02_set_face_detect_pre_value(client,
			g_param_ft5x02_qtw.ft5x02_FACE_DETECT_PRE_VALUE);
	if (err < 0) {
		dev_err(&client->dev,
				"%s:could not set face detect pre value.\n",
				__func__);
	...
	/*set face detect num*/
	err = ft5x02_set_face_detect_num(client,
			g_param_ft5x02_qtw.ft5x02_FACE_DETECT_NUM);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set face detect num.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set face detect last time*/
	err = ft5x02_set_face_detect_last_time(client,
			g_param_ft5x02_qtw.ft5x02_FACE_DETECT_LAST_TIME>>8,
			g_param_ft5x02_qtw.ft5x02_FACE_DETECT_LAST_TIME);
	if (err < 0) {
		dev_err(&client->dev,
			"%s:could not set face detect last time.\n",
			__func__);
	...
	/*set min peak value*/
	err = ft5x02_set_peak_value_min(client,
			g_param_ft5x02_qtw.ft5x02_BIGAREA_PEAK_VALUE_MIN);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set min peak value.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set diff value over num*/
	err = ft5x02_set_diff_value_over_num(client,
			g_param_ft5x02_qtw.ft5x02_BIGAREA_DIFF_VALUE_OVER_NUM);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set diff value over num.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set point auto clear time*/
	err = ft5x02_set_point_auto_clear_time(client,
			g_param_ft5x02_qtw.ft5x02_BIGAREA_POINT_AUTO_CLEAR_TIME);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set point auto clear time.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set kx*/
	err = ft5x02_set_kx(client, g_param_ft5x02_qtw.ft5x02_KX);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set kx.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set ky*/
	err = ft5x02_set_ky(client, g_param_ft5x02_qtw.ft5x02_KY);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set ky.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set lemda x*/
	err = ft5x02_set_lemda_x(client,
			g_param_ft5x02_qtw.ft5x02_LEMDA_X);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set lemda x.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set lemda y*/
	err = ft5x02_set_lemda_y(client,
			g_param_ft5x02_qtw.ft5x02_LEMDA_Y);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set lemda y.\n",
			__func__);
		goto ERR_EXIT;
	...
	/*set pos x*/
	err = ft5x02_set_pos_x(client, g_param_ft5x02_qtw.ft5x02_DIRECTION);
	if (err < 0) {
		dev_err(&client->dev, "%s:could not set pos x.\n",
			__func__);
		goto ERR_EXIT;
