...
#ifdef CONFIG_LEDS_SUNXI_PB624
#define SCRIPT_SECTION_LED "LED_para"
#define SCRIPT_KEY_LED_USED "LED_used"
#define SCRIPT_KEY_LED_GPIO "GPIO_LED_PIN"
#define LED_NAME_PB624 "power_led"
#else
#define SCRIPT_SECTION_LED "leds_para"
#define SCRIPT_KEY_LED_USED "leds_used"
#endif
...
static int __init sunxi_leds_init(void)
{
	...
	err = script_parser_fetch(SCRIPT_SECTION_LED, SCRIPT_KEY_LED_USED, &sunxi_leds_used, sizeof(sunxi_leds_used)/sizeof(int));
	if (err) {
		/* Not error - just info */
...
#ifndef CONFIG_LEDS_SUNXI_PB624
	err = script_parser_fetch(SCRIPT_SECTION_LED, "leds_num", &sunxi_leds_num,
					sizeof(sunxi_leds_num)/sizeof(int));
	if (err) {
		pr_err("%s script_parser_fetch '[leds_para]' 'leds_num' error\n",__func__);
		return err;
	}
#else //CONFIG_LEDS_SUNXI_PB624
	//for PB624 project use one
	sunxi_leds_num = 1;
#endif
	if (!sunxi_leds_num) {
	...
	for (i = 0; i < sunxi_leds_num; i++) {
		...
		err = script_parser_fetch(SCRIPT_SECTION_LED, key,
					  (int *)leds_i->led_name,
		...
		leds_i->gpio_handler = gpio_request_ex(SCRIPT_SECTION_LED, key);
		...
		err = script_parser_fetch(SCRIPT_SECTION_LED, key, &value, 1);
		if (err == 0)
...
#else //CONFIG_LEDS_SUNXI_PB624
	strcpy(leds_i->led_name,LED_NAME_PB624);
	leds_i->gpio_handler = gpio_request_ex(SCRIPT_SECTION_LED, SCRIPT_KEY_LED_GPIO);
	if (!leds_i->gpio_handler) {
		pr_err("%s can't request '[leds_para]' '%s', already used ?",
				__func__, key);
		goto exit;
	...
	dleds_i->name = leds_i->led_name;
	dleds_i->default_state = LEDS_GPIO_DEFSTATE_ON;
	dleds_i->gpio = 0;
	dleds_i->default_trigger = "none";
	dleds_i->active_low = 1;
#endif //CONFIG_LEDS_SUNXI_PB624
