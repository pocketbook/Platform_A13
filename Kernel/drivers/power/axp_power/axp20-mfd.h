...
extern void set_poweroff_flag(int flag);

static ssize_t axp20_poweroff_flag_show(struct device *dev, struct device_attribute *attr, char *buf) {
	int n = 0;
	int flag = get_poweroff_flag();
	n = sprintf(buf,"flag = %i\n",flag);
	return n;
}

static ssize_t axp20_poweroff_flag_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
	int flag = 0;
	flag = simple_strtoul(buf, NULL, 10);
	set_poweroff_flag(flag);
	return count;
}
...
static struct device_attribute axp20_mfd_attrs[] = {
	...
	AXP_MFD_ATTR(axp20_poweroff_flag),
};
