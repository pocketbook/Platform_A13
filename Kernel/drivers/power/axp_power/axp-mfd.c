...
void set_poweroff_flag (int flag) {
	power_start = flag;
}
...
static void axp_power_off(void)
{
    ...
    if (power_start == 3 || power_start == 1) {
    	int ret = 0;
    	uint8_t val = 0;
    	uint8_t val_array[8];
    	int count = 0;
    	bool reduce_cur_flag = false;

    	if ((axp_read(&axp->dev,POWER20_STATUS, &val) < 0) || !(val & (AXP20_STATUS_ACEN | AXP20_STATUS_USBEN))) {
    		printk("[axp] No external power sources!\n");

    	} else {
    		printk("[axp] full charge first and then send power-off command!\n");
    		disable_irq(axp->irq);
    		//clean PEK interrupt status
    		ret = axp_set_bits(&axp->dev,POWER20_INTSTS3,((1 << 0) | (1 << 1)));

    		int tics = 0;
    		const char *p = "..........";

#if defined (CONFIG_AXP_NO_POWEROFF_IF_USB_ON)
    		//set powersave mode
    		...
    		int charging_finished = 0;
#endif

    		while (true) {

#if defined (CONFIG_AXP_NO_POWEROFF_IF_USB_ON)
    			if (!charging_finished)
#endif
    				printk("<c>[axp] Charging %.*s                 \r",tics++ % 10,p);

    			//check external power status
    			ret = axp_read(&axp->dev,POWER20_STATUS, &val);
    			if (ret < 0) {
    				printk("[axp] i2c read error %i!\n", ret);
    				break;
    			}

    			if(!(val & (AXP20_STATUS_ACEN | AXP20_STATUS_USBEN))) {
    				printk("[axp] External source was disabled!\n");
    				break;
    			}

    			//check charging process
    			ret = axp_read(&axp->dev,POWER20_MODE_CHGSTATUS, &val);
    			if (ret < 0) {
    				printk("[axp] i2c read error %i!\n", ret);
    				break;
    			}
    			...
    			if ((val & (1 << 7))) {
    				printk("[axp] AXP chip is over-temperature! Will be power off\n");
    				break;
    			}

    			//check power key
    			ret = axp_read(&axp->dev,POWER20_INTSTS3, &val);
    			if (ret < 0) {
    				printk("[axp] i2c read error %i!\n", ret);
    				break;
    			}
    			...
    			if (val & ((1 << 0) | (1 << 1))) {
    				printk("[axp] PEK was pressed!\n");
    				goto reboot;
    				break;
    			}

    			//current monitor
    			ret = axp_reads(&axp->dev,POWER20_BAT_AVERCHGCUR_H8, 2, val_array);
    			if (ret == 0) {
    				int mic = (((int)val_array[0] << 4) | (val_array[1] & 0xF)) / 2;
    				//printk("[axp] ic=%i\n",mic);
    				if (mic < 300 && !reduce_cur_flag) {
    					if (++count > 10) {
    						axp_update(&axp->dev,POWER20_CHARGE1,0x00,0x0F);
    						reduce_cur_flag = true;
    					}
    				} else
    					count = 0;
