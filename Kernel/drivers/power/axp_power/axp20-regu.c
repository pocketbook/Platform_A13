...
#if defined(CONFIG_RTL8188_POWER_ONOFF) || defined(CONFIG_USB_SW_SUN5I_HCI_AXP_POWER_CONTROL)
static struct regulator_dev *ldo3 = NULL;
void disable_ldo3(void) {
	if (ldo3 == NULL) {
		printk("[%s] ldo3 is not finded\n",__FUNCTION__);
		return;
	}
	axp_disable(ldo3);
	printk("[%s] LDO3 disabled\n",__FUNCTION__);
}

void enable_ldo3(void) {
	if (ldo3 == NULL) {
		printk("[%s] ldo3 is not finded\n",__FUNCTION__);
		return;
	}
	axp_enable(ldo3);
	printk("[%s] LDO3 enabled\n",__FUNCTION__);
}
EXPORT_SYMBOL(disable_ldo3);
EXPORT_SYMBOL(enable_ldo3);
#endif
...
#if defined(CONFIG_RTL8188_POWER_ONOFF) || defined(CONFIG_USB_SW_SUN5I_HCI_AXP_POWER_CONTROL)
	//disable usb host and wifi power
	if (ri->desc.id == AXP20_ID_LDO3) {
		printk("[%s] disable AXP20_ID_LDO3\n",__FUNCTION__);
		ldo3 = rdev;
		axp_disable(ldo3);
	}
#endif
